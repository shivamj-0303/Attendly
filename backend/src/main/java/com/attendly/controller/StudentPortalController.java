...
public class StudentPortalController {
    ...
    public ResponseEntity<List<TimetableSlotResponse>> getTimetable(
        ...
        // Get student details
        Student student = studentRepository.findById(userPrincipal.getId())
        ...
        if (date != null) {
            ...
            List<TimetableSlotResponse> slots = timetableService.getStudentTimetableByDay(
                    student.getClassId(), dayName);
            return ResponseEntity.ok(slots);
        } else {
            // Get full week timetable
            List<TimetableSlotResponse> slots = timetableService.getStudentTimetable(student.getClassId());
            return ResponseEntity.ok(slots);
        }
    }
    ...
    public ResponseEntity<List<AttendanceResponse>> getTodayAttendance(
            @AuthenticationPrincipal UserPrincipal userPrincipal) {
        List<AttendanceResponse> attendance = attendanceService.getTodayAttendanceForStudent(userPrincipal.getId());
