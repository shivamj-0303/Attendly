...
public class StudentService {
  ...
  public Student createStudent(StudentRequest request, Long adminId) {
    ...
    if (rollNumber == null || rollNumber.isEmpty()) {
      // Find highest roll number in the class
      List<Student> existingStudents = studentRepository.findByClassIdOrderByRollNumber(request.getClassId());
      int maxRollNumber = 0;
      for (Student s : existingStudents) {
        try {
          int rollNum = Integer.parseInt(s.getRollNumber().replaceAll("[^0-9]", ""));
          if (rollNum > maxRollNumber) {
            maxRollNumber = rollNum;
          }
        } catch (NumberFormatException ignored) {
          // Skip non-numeric roll numbers
        }
      }
      rollNumber = String.valueOf(maxRollNumber + 1);
    } else {
    ...
    if (generated) {
      String subject = "Welcome to Attendly — your account details";
      String html = String.format(
          "<p>Hello %s,</p><p>Your account has been created. Use the following temporary password to sign in:</p><p><strong>%s</strong></p><p>You can change this password anytime using the password reset feature.</p>",
  ...
  @Transactional
  public List<Student> importStudentsBulk(
      com.attendly.dto.BulkStudentImportRequest request, Long adminId) {
    // Validate non-empty
    if (request.getStudents() == null || request.getStudents().isEmpty()) {
      throw new IllegalArgumentException("No students provided for import");
    }

    // Collect emails and roll numbers to validate duplicates within payload
    java.util.Set<String> emails = new java.util.HashSet<>();
    java.util.Set<String> rolls = new java.util.HashSet<>();
    for (com.attendly.dto.BulkStudentImportRequest.StudentItem it : request.getStudents()) {
      if (it.getEmail() == null || it.getEmail().isEmpty())
        throw new IllegalArgumentException("Each student must have an email");
      if (it.getName() == null || it.getName().isEmpty())
        throw new IllegalArgumentException("Each student must have a name");
      if (it.getPhone() == null || it.getPhone().isEmpty())
        throw new IllegalArgumentException("Each student must have a phone number");

      if (!emails.add(it.getEmail().toLowerCase())) {
        throw new com.attendly.exception.ResourceAlreadyExistsException(
            "Duplicate email in import file: " + it.getEmail());
      }
      
      // Only check roll number duplicates if provided
      if (it.getRollNumber() != null && !it.getRollNumber().isEmpty()) {
        if (!rolls.add(it.getRollNumber())) {
          throw new com.attendly.exception.ResourceAlreadyExistsException(
              "Duplicate roll number in import file: " + it.getRollNumber());
        }
      }
    }

    // Check DB for existing emails/rolls
    for (String e : emails) {
      if (studentRepository.existsByEmail(e)) {
        throw new com.attendly.exception.ResourceAlreadyExistsException("Student email exists: " + e);
      }
    }
    for (String r : rolls) {
      if (studentRepository.existsByRollNumber(r)) {
        throw new com.attendly.exception.ResourceAlreadyExistsException(
            "Student roll number exists: " + r);
      }
    }

    // Verify classes exist and belong to admin for each unique classId
    java.util.Set<Long> classIds = new java.util.HashSet<>();
    for (com.attendly.dto.BulkStudentImportRequest.StudentItem it : request.getStudents()) {
      classIds.add(it.getClassId());
    }
    for (Long cid : classIds) {
      classRepository
          .findByIdAndAdminId(cid, adminId)
          .orElseThrow(() -> new com.attendly.exception.ResourceNotFoundException("Class not found: " + cid));
    }

    // Find the highest existing roll number for auto-generation
    // Get all students in the relevant classes and find max roll number
    int maxRollNumber = 0;
    for (Long cid : classIds) {
      List<Student> existingStudents = studentRepository.findByClassIdOrderByRollNumber(cid);
      for (Student s : existingStudents) {
        try {
          // Try to parse roll number as integer
          int rollNum = Integer.parseInt(s.getRollNumber().replaceAll("[^0-9]", ""));
          if (rollNum > maxRollNumber) {
            maxRollNumber = rollNum;
          }
    ...
    // Start auto-generated roll numbers from next available
    int nextRollNumber = maxRollNumber + 1;

    // Create students and send emails for generated passwords
    java.util.List<Student> created = new java.util.ArrayList<>();
    for (com.attendly.dto.BulkStudentImportRequest.StudentItem it : request.getStudents()) {
      String initialPassword = generateSecurePassword(10);
      
      // Auto-generate roll number if not provided
      String rollNumber = it.getRollNumber();
      if (rollNumber == null || rollNumber.isEmpty()) {
        rollNumber = String.valueOf(nextRollNumber++);
      }
      ...
      Student student =
          Student.builder()
              ...
              .phone(it.getPhone())
              .rollNumber(rollNumber)
              .registrationNumber(it.getRegistrationNumber())
