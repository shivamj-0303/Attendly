...
public class TeacherService {
    ...
    public Teacher createTeacher(TeacherRequest request, Long adminId) {
        ...
        if (teacherRepository.existsByEmail(request.getEmail())) {
            throw new ResourceAlreadyExistsException("Teacher with email " + request.getEmail() + " already exists");
        }

        // Verify department exists
        if (request.getDepartmentId() != null) {
            departmentRepository.findByIdAndAdminId(request.getDepartmentId(), adminId)
    ...
    public List<Teacher> getTeachersByDepartment(Long departmentId, Long adminId) {
        ...
        departmentRepository.findByIdAndAdminId(departmentId, adminId)
    ...
    public Teacher getTeacherById(Long id, Long adminId) {
        return teacherRepository.findByIdAndAdminId(id, adminId)
    ...
    public Teacher updateTeacher(Long id, TeacherRequest request, Long adminId) {
        ...
        if (request.getDepartmentId() != null && !request.getDepartmentId().equals(teacher.getDepartmentId())) {
            departmentRepository.findByIdAndAdminId(request.getDepartmentId(), adminId)
                    .orElseThrow(() -> new ResourceNotFoundException("Department not found with id: " + request.getDepartmentId()));
        }
    ...
    public List<Teacher> searchTeachersInDepartment(String query, Long departmentId) {
        return teacherRepository.findByDepartmentIdAndNameContainingIgnoreCaseOrEmailContainingIgnoreCase(
