...
public class TimetableService {
    ...
    public TimetableSlotResponse createSlot(TimetableSlotRequest request, Long adminId) {
        ...
        List<TimetableSlot> existingSlots = timetableSlotRepository
                .findByClassIdAndDayOfWeekAndIsActiveTrue(request.getClassId(), request.getDayOfWeek());
        
        for (TimetableSlot slot : existingSlots) {
            if (hasTimeOverlap(request.getStartTime(), request.getEndTime(), 
                              slot.getStartTime(), slot.getEndTime())) {
                throw new IllegalArgumentException(
                    "Time slot overlaps with existing slot: " + slot.getSubject() + 
                    " (" + slot.getStartTime() + " - " + slot.getEndTime() + ")"
                );
            }
        }
    ...
    public List<TimetableSlotResponse> getSlotsByClass(Long classId, Long adminId) {
        ...
        return timetableSlotRepository.findByClassIdAndIsActiveTrue(classId)
                .stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }

    public List<TimetableSlotResponse> getSlotsByClassAndDay(Long classId, String dayOfWeek, Long adminId) {
        // Verify class belongs to admin
        classRepository.findByIdAndAdminId(classId, adminId)
                .orElseThrow(() -> new ResourceNotFoundException("Class not found with id: " + classId));

        return timetableSlotRepository.findByClassIdAndDayOfWeekAndIsActiveTrue(classId, dayOfWeek)
                .stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
    ...
    public TimetableSlotResponse updateSlot(Long id, TimetableSlotRequest request, Long adminId) {
        ...
        // Verify class belongs to admin
        classRepository.findByIdAndAdminId(slot.getClassId(), adminId)
        ...
        for (TimetableSlot existingSlot : existingSlots) {
            if (!existingSlot.getId().equals(id) && 
                hasTimeOverlap(request.getStartTime(), request.getEndTime(), 
                              existingSlot.getStartTime(), existingSlot.getEndTime())) {
                throw new IllegalArgumentException(
                    "Time slot overlaps with existing slot: " + existingSlot.getSubject() + 
                    " (" + existingSlot.getStartTime() + " - " + existingSlot.getEndTime() + ")"
                );
            }
        }
